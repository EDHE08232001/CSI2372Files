...
{
protected:
    std::vector<Card *> chain; /**< Vector holding pointers to cards in the chain */
...
public:
    ...
    /**
     * @brief Adds a card to the chain.
     *        Throws an exception if the card type does not match the chain's bean type.
     * @param card Pointer to the card to add.
    ...
    {
        if (getSize() == 0)
            chainType = typeid(*card).name(); // Update the chain type based on the first card
